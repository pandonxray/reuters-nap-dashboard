...
看板会在所有页面顶部检查当前视图是否更新到预计的最近工作日，并在“资料｜数据健康”中列出具体序列、最后日期、滞后工作日和近期缺失日期。数据健康页还会单独显示：

- 连续月 M1-M12 的缺月，例如某个 RIC 族缺少 M3；
- 同一工作表内重复请求同一 Reuters RIC 的源公式冲突；
- 晚于本机日期的未来数据行；这些行保留用于核对，但不会进入行情、图表、价差或风险计算。

遇到重复 RIC 时，解析器只保留合约标签与 RIC 月份一致的序列，并隔离冲突序列。这样其他已刷新的数据仍能加载，同时缺失合约会明确报错，不会被错误月份替代。修正 Excel 源单元格并完成 LSEG 刷新后，点击“重新解析当前 Excel”即可恢复完整曲线。
